...
MLexport <- read.csv("MLexport.csv")
df = MLexport
head(df)
df_agg = df %>% group_by(df[6],df[7],df[9],df[10]) %>% summarise(number_trips = n())

#df_agg <- df_agg %>% add_column(is_summer = if_else(.$MonthInt==6|.$MonthInt==7|.$MonthInt==8, 1, 0))
df_agg <- df_agg %>% add_column(is_jan = if_else(.$MonthInt==1, 1, 0))
df_agg <- df_agg %>% add_column(is_feb = if_else(.$MonthInt==2, 1, 0))
df_agg <- df_agg %>% add_column(is_mar = if_else(.$MonthInt==3, 1, 0))
df_agg <- df_agg %>% add_column(is_apr = if_else(.$MonthInt==4, 1, 0))
df_agg <- df_agg %>% add_column(is_may = if_else(.$MonthInt==5, 1, 0))
df_agg <- df_agg %>% add_column(is_jun = if_else(.$MonthInt==6, 1, 0))
df_agg <- df_agg %>% add_column(is_jul = if_else(.$MonthInt==7, 1, 0))
df_agg <- df_agg %>% add_column(is_aug = if_else(.$MonthInt==8, 1, 0))
df_agg <- df_agg %>% add_column(is_sep = if_else(.$MonthInt==9, 1, 0))
df_agg <- df_agg %>% add_column(is_oct = if_else(.$MonthInt==10, 1, 0))
df_agg <- df_agg %>% add_column(is_nov = if_else(.$MonthInt==11, 1, 0))
df_agg <- df_agg %>% add_column(is_dec = if_else(.$MonthInt==12, 1, 0))

df_agg <- df_agg %>% add_column(is_long_trip = if_else(.$Trip.Duration.Minutes>median(df_agg$Trip.Duration.Minutes), 1, 0))

head(df_agg)


###df_chng = df_agg %>% filter(is_long_trip == 0)
library(caTools)
set.seed(1)
sampled = sample(c(TRUE, FALSE), nrow(df_agg), replace=TRUE, prob=c(0.7,0.3)) #Splitting it into 70-30
train_set=df_agg[sampled, ]
test_set=df_agg[!sampled, ]
...
traintemp = tree(number_trips~Lat,data=train_set,mindev=.0001)
cat('first big tree size: \n')
print(length(unique(traintemp$where)))

#then prune it down to one with 7 leaves
train.tree=prune.tree(traintemp,best=7)
...
#plot the tree
plot(train.tree,type="uniform")
text(train.tree,col="blue",label=c("yval"),cex=.8)

#plot data with fit
train.fit = predict(train.tree) #get training fitted values
plot(train_set$Lat,train_set$number_trips,cex=.5,pch=16) #plot data
oo=order(train_set$Lat)
lines(train_set$Lat[oo],train.fit[oo],col='red',lwd=3) #step function fit
print(train.tree$splits)
cvals=c(30.281,30.273,30.2639,30.2671,30.2836,30.2948) #cutpoints from tree
...
################################################################################
## Fit a regression tree to medv~dis+lstat from the Boston housing data.
...
print(nset)
for(i in 1:nset) {
   cat('doing train_set rf: ',i,'\n')
   rffit = randomForest(number_trips~Lat,data=train_set,ntree=ntreev[i],maxnodes=15)
